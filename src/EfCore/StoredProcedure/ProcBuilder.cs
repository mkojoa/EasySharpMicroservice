...
{
    internal class ProcBuilder : IProcBuilder
    {
        private const string RetParamName = "_retParam";
        private readonly DbCommand _cmd;

        public ProcBuilder(DbContext ctx, string name)
        {
            if (name is null)
        ...
        public IProcBuilder AddParam<T>(string name, T val)
        {
            AddParamInner(name, val, ParameterDirection.Input);
            return this;
        }

        public IProcBuilder AddParam<T>(string name, out IOutParam<T> outParam)
        {
        ...
        public IProcBuilder AddParam<T>(string name, out IOutParam<T> outParam, int size = 0, byte precision = 0, byte scale = 0)
        {
            outParam = AddOutputParamInner(name, default(T), ParameterDirection.Output, size, precision, scale);
            return this;
        }

        public IProcBuilder AddParam<T>(string name, T val, out IOutParam<T> outParam)
        {
            outParam = AddOutputParamInner(name, val, ParameterDirection.InputOutput);
            return this;
        }

        public IProcBuilder AddParam(DbParameter parameter)
        {
            if (parameter == null)
            {
        ...
        public IProcBuilder ReturnValue<T>(out IOutParam<T> retParam)
        {
            retParam = AddOutputParamInner(RetParamName, default(T), ParameterDirection.ReturnValue);
            return this;
        }

        public IProcBuilder SetTimeout(int timeout)
        {
